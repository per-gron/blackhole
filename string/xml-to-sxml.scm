...
; ####
; ## Some routines that can as well be left in the global namespace (they don't use anything closure-internal)
...
;; return #f for those
    ...
    (define (ssax:warn . args)
      ...
      ;      (cdr args) nl)
      ;(begin
      (print nl "Warning " ; (port-name (car args))
             )
      (try-cerr-at-position)
      (print ": " (cdr args) nl))
...
; Top-level cond-expand expanded automatically
    ...
    (let* ((ascii->char integer->char)
           (ssax:S-chars (map ascii->char '(32 10 9 13))) ; of SSAX-code.scm
           )
